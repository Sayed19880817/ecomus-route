...
export default function SingleCategoryProducts() {
  ...
  const [currentCategory, setCurrentCategory] = useState<CategoryAndBrand>();
  ...
  async function fetchSingleCategoryData() {
    const data = await apiServices.getSpecificCategory(String(catId));
    setCurrentCategory(data.data);
  }

  useMemo(() => fetchSingleCategoryData(), []);
  ...
  useEffect(() => {
    document.title = currentCategory?.name + " - " + "Products - Ecomus";
  }, [currentCategory]);
